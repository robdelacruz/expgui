...
static void db_assign_cat_names(db_t *db);
static void db_assign_exp_cat_name(db_t *db, exp_t *xp);
...
static void db_assign_cat_names(db_t *db) {
    ...
    for (int i=0; i < db->xps->len; i++) {
        db_assign_exp_cat_name(db, db->xps->items[i]);
    }
}
static void db_assign_exp_cat_name(db_t *db, exp_t *xp) {
    cat_t *cat = db_find_cat(db, xp->catid);
    if (cat != NULL)
