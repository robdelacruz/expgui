...
uictx_t *uictx_new() {
    ...
    array_assign(ctx->all_xps_array,
                 ctx->all_xps, 0, countof(ctx->all_xps));
    array_assign(ctx->view_xps_array,
                 ctx->view_xps, 0, countof(ctx->view_xps));
...
void filter_expenses(uictx_t *ctx) {
    ...
    array_t *xps = ctx->all_xps_array;
    array_t *view_xps = ctx->view_xps_array;
